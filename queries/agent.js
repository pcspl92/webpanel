...
const rechargeSubAgent = (subAgentId, amount) => {
  const sql1 = `UPDATE agents_add_data SET balance=balance+${amount} WHERE agent_id=${subAgentId};`;
  const sql2 = `UPDATE agents_add_data SET balance=balance-${amount} WHERE agent_id=(SELECT agent_id FROM agents WHERE id=${subAgentId})`;
